...
// src/index.ts
var AWC_BASE = "https://aviationweather.gov/api/data";
var METAR_STATIONS = ["KMCI", "KJFK", "KLAX", "KORD", "KATL", "KDFW", "EGLL", "LFPG", "EDDF", "RJTT", "YSSY", "VIDP"];
var PLUGIN_ID = "aviation-weather-hazards";
var SNAPSHOT_TTL_SECONDS = 2700;
function finiteOrNull(value) {
  return typeof value === "number" && Number.isFinite(value) ? value : null;
}
function mapMetarToItem(metar) {
  const lat = metar.lat;
  const lon = metar.lon;
...
function computePolygonCentroid(coords) {
  ...
  const vertices = first && last && first.lat === last.lat && first.lon === last.lon && coords.length > 1 ? coords.slice(0, -1) : coords;
  let latSum = 0;
  let lonSum = 0;
  let count = 0;
...
function mapSigmetToItem(sigmet) {
...
async function seedAviationWeatherHazards() {
  ...
  try {
    ...
    const metarPayload = await metarRes.json();
    const sigmetPayload = await sigmetRes.json();
    ...
    if (Array.isArray(sigmetPayload)) {
      for (const sigmet of sigmetPayload) {
        ...
        if (!item) continue;
        items.push(item);
        const sourceTs = typeof sigmet.validTimeFrom === "number" ? sigmet.validTimeFrom * 1e3 : fetchedAt;
        const result = insertHazard.run({
          id: item.id,
          payload: JSON.stringify(item),
          source_ts: sourceTs,
