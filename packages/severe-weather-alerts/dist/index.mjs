...
// src/index.ts
var NWS_ALERTS_URL = "https://api.weather.gov/alerts/active";
var NWS_USER_AGENT = "wwv-plugin/1.0 (contact: batch@worldwideview.dev)";
var PLUGIN_ID = "severe-weather-alerts";
var SNAPSHOT_TTL_SECONDS = 1800;
function polygonRepresentativePoint(geometry) {
  ...
  const lon = (sumLon / n + 540) % 360 - 180;
  return { lat, lon };
}
function parseActiveAlerts(payload) {
  ...
  for (const raw of features) {
    ...
    if (!point) continue;
    const p = (_a = feature.properties) != null ? _a : {};
    items.push({
      id: (_c = (_b = p.id) != null ? _b : feature.id) != null ? _c : `nws-${items.length}`,
      lat: point.lat,
      lon: point.lon,
      event: (_d = p.event) != null ? _d : null,
      severity: (_e = p.severity) != null ? _e : null,
      urgency: (_f = p.urgency) != null ? _f : null,
      headline: (_g = p.headline) != null ? _g : null,
      description: (_h = p.description) != null ? _h : null,
      instruction: (_i = p.instruction) != null ? _i : null,
      sent: (_j = p.sent) != null ? _j : null,
      effective: (_k = p.effective) != null ? _k : null,
      expires: (_l = p.expires) != null ? _l : null,
      areaDesc: (_m = p.areaDesc) != null ? _m : null
    });
  }
  return items;
}
function alertEpochMs(item) {
  var _a;
  const value = (_a = item.sent) != null ? _a : item.effective;
  if (value) {
...
async function seedSevereWeatherAlerts() {
  try {
